  ...
  for (const route of routesPaths) {
    queue.add(async () => {
      try {
        ...
        // render head using Unhead's SSR renderer
        const template = jsdom.serialize()
        const html = head ? transformHtmlTemplate(head as ReturnType<typeof createHead>, template) : template
